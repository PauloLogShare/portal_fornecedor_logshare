...
export default function CarrierPortal({ onDossierSubmitted, carriers = [], isStandalone = false, onOpenSpecialistLogin }) {
  ...
  const handleFinalSubmit = () => {
    ...
    if (!hasReceitaDoc && (formData.dadosReceitaFederal || formData.cnpj)) {
      try {
        const rfPdf = generateReceitaFederalPDF({ ...formData, protocol });
        docs.unshift({
        ...
        console.warn("Could not generate Receita Federal PDF on submit", pdfErr);
      }
    }
